# Holistic decision analysis model of silvoarable mixed fruit syntropic agroforestry system of Hof Lebensberg ####
# 1632 trees: Apples, Pear, Medlar, Quince, Nashi-pear, Plum, Cherry, Peach, Apricot, Figs. 
# 1150 considered in this script
# Since majority are apple trees, yield and harvest time are focused on that 
# Introduce vegetables (potatoes) after 3 years instead of cereals??
...
AF_benefit <- function(x, varnames)
{
  ...
  #Tree component - fruit in AF system
  AF_nuts_yield <- rep(0, n_years)
  ES3_subsidy <- rep(0, n_years)
  
  #Yield of one fruit tree [kg/tree]
  AF_fruit_yield <- rep(0, n_years)
  AF_fruit_yield <- gompertz_yield(max_harvest = fruit_yield_max,
                                   time_to_first_yield_estimate = time_to_first_fruit,
                                   time_to_second_yield_estimate = time_to_second_fruit,
                                   first_yield_estimate_percent = fruit_yield_first,
                                   second_yield_estimate_percent = fruit_yield_second,
                                   n_years=n_years,
                                   var_CV = var_cv,
  ...
  Lower_qual_fruit_yield <- AF_tot_fruit_yield * (1-Pc_table_fruit) #rest of yield is classified as lower quality
  Pc_b_qual_fruit <- vv(perc_bqual_fruit, var_CV = var_cv, n_years)/100 #B-quality fruits can still be sold in direct selling operation, but at significantly lower price
  B_qual_table_fruit_yield <- Lower_qual_fruit_yield * Pc_b_qual_fruit #amount of B-quality fruits [kg]
  Juice_fruit_yield <- Lower_qual_fruit_yield * (1-Pc_b_qual_fruit) #Rest of the fruit yield can be sold as juicing fruit at lowest price 
  
  #The benefits from table apples and juice apples are calculated by multiplying their yields by their respective prices  
  Table_fruit_benefit <- Table_fruit_yield * table_fruit_price
  B_qual_fruit_benefit <- B_qual_table_fruit_yield * bqual_fruit_price
  Juice_fruit_benefit <-  Juice_fruit_yield * juice_fruit_price
  
  AF_fruit_benefit <- Table_fruit_benefit + B_qual_fruit_benefit + Juice_fruit_benefit
  ...
  AF_tree_benefit <- AF_fruit_benefit + Nonmarket_ES_benefit
  ...
  AF_fruit_harvest <-  rep(0, n_years) #FE; Labour hours of mowing the tree rows manually [h/ha]
  ...
  AF_fruit_harvest[time_to_first_fruit:n_years] <-
    vv(fruit_harvest, var_CV = var_cv, length(time_to_first_fruit:n_years)) # labour cost of harvesting
    #Total cost of tree component in AF system
  AF_total_treerow_management_cost <- ES3_application + AF_pruning + AF_root_pruning + 
    AF_annual_irrigation_cost + AF_mowing_treerow + AF_fruit_harvest
